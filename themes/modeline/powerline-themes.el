                ...
                '("%e"
                  (:eval
                   (let* ((active (powerline-selected-window-active))
                          ...
                          (lhs (list (cond (evil-insert-state-minor-mode
                                            (powerline-raw evil-insert-state-tag face4))
                                           (evil-visual-state-minor-mode
                                            (powerline-raw evil-visual-state-tag face4)))

                                     (when multiple-cursors-mode
                                       (powerline-raw (concat " mc:" (number-to-string (mc/num-cursors)) " ") face4))
                                     ...
                                     (let ((project-name (projectile-project-name)))
                                       (unless (string= "-" project-name)
                                         (powerline-raw (concat " in " (projectile-project-name)) 'powerline-inactive0)))

				     (when (vc-mode)
				       (powerline-raw '(vc-mode vc-mode) 'powerline-active2))
                          ...
                          (rhs (list
                                (powerline-raw "%l" face5 'l)
                                (powerline-raw "," face5 'l)
                                (powerline-raw " %c " face5 'r)
                                (powerline-raw (concat " "
						       (getenv "MODELINE") " ") face3))))
                  ...
                  (:eval
                   (let* ((active (powerline-selected-window-active))
                          ...
                          (separator-right (intern (format "powerline-%s-%s"
                                                           (powerline-current-separator)
                                                           (cdr powerline-default-separator-dir))))
                          (lhs (list (powerline-buffer-id `(mode-line-buffer-id ,face1) 'l)
                                     (when (buffer-modified-p)
                                       (powerline-raw "[+] " face0))
                                     (when buffer-read-only
